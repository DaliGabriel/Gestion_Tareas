...
<div class="antialiased sans-serif bg-gray-100 h-screen">
	<div x-data="app()" x-init="[initDate(), getNoOfDays()]" x-cloak>
		<div class="container mx-auto px-4 py-2 md:py-24">
			...
			<div class="bg-white rounded-lg shadow overflow-hidden">
				...
				<div class="flex items-center justify-between py-8 px-6">
					<div>
					...
					<div class="flex flex-wrap border-t border-l">
						...
						<template x-for="(date, dateIndex) in no_of_days" :key="dateIndex">	
							<div style="width: 14.28%; height: 120px" class="px-4 pt-2 border-r border-b relative">
								<div
									@click="showEventModal(date)borrar esto para activar el modal" 
									x-text="date"
		...
		<div style=" background-color: rgba(0, 0, 0, 0.8)" class="fixed z-40 top-0 right-0 left-0 bottom-0 h-full w-full" x-show.transition.opacity="openEventModal">
			<div class="p-4 max-w-xl mx-auto relative absolute left-0 right-0 overflow-hidden mt-24">
				...
				<div class="shadow w-full rounded-lg bg-white overflow-hidden w-full block p-8">
					...
					<h2 class="font-bold text-2xl mb-6 text-gray-800 border-b pb-2">Agregar Tarea</h2>
					...
					<div class="inline-block w-64 mb-4">
						<label class="text-gray-800 block mb-1 font-bold text-sm tracking-wide">Escoje un color</label>
						<div class="relative">
	...
	<script>
		...
		const MONTH_NAMES = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octuber', 'Noviembre', 'Diciembre'];
		const DAYS =['', '', '', '', '', '', ''];
		...
		function app() {
			return {
				...
				events: [
					...
					{
						event_date: new Date({!! json_encode($fecha->format('Y,m,d')) !!}),
						event_title: {!! json_encode($tarea->titulo) !!},
						event_theme: 'red'
					},
					@endforeach
				],
